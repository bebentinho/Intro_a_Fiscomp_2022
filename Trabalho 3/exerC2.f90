...
theta_0 = theta_0 * pi / 180.0d0 !convertendo para rad

Nlinhas = int(T/dt)

open(unit = 1, file = "exerC2_out.dat")
open(unit = 2, file = "exerC2ener_out.dat") !arquivo para o gráfico da energia
...
do i = 1, Nlinhas
    ...
    if (theta + w * dt > 1.d0 * pi) then
        theta = theta + w * dt - 2.d0 * pi

    else if (theta + w * dt < -1.d0 * pi) then
        theta = theta + w * dt + 2.d0 * pi
